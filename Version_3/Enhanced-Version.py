...
def modified_order_crossover(parent1, parent2, crossover_rate=0.8):
    """
    Perform the Modified Order Crossover (MOX) on two parent chromosomes.

    This operator is based on the enhancement of the traditional Order Crossover (OX),
    as proposed in the paper "Enhanced Order Crossover for Permutation Problems"
    (IJIRSET, Vol. 4, Issue 2, February 2015).

    Instead of selecting two random crossover points as in OX, MOX chooses a fixed 
    swath (substring) length l = max(3, β), where β is randomly selected in the range [n/9, n/7].

    Parameters:
    - parent1: list[int] -- The first parent chromosome (a permutation).
    - parent2: list[int] -- The second parent chromosome (a permutation).
    - crossover_rate: float -- Probability of applying crossover.

    Returns:
    - offspring1: list[int] -- The first offspring.
    - offspring2: list[int] -- The second offspring.
    """
    if random.random() >= crossover_rate:
        return parent1, parent2  # No crossover applied

    n = len(parent1)
    beta = random.randint(n // 9, n // 7)
    l = max(3, beta)

    # Ensure the swath fits within the chromosome
    point1 = random.randint(0, n - l)
    point2 = point1 + l

    # Create offspring1
    segment1 = parent1[point1:point2]
    remainder1 = [gene for gene in parent2 if gene not in segment1]
    offspring1 = remainder1[:point1] + segment1 + remainder1[point1:]

    # Create offspring2
    segment2 = parent2[point1:point2]
    remainder2 = [gene for gene in parent1 if gene not in segment2]
    offspring2 = remainder2[:point1] + segment2 + remainder2[point1:]
...
NUM_ISLANDS = 4
MIGRATION_INTERVAL = 30 # generations
    ...
    for generation in range(max_generations):
        ...
        for i in range(NUM_ISLANDS):
            ...
            for _ in range(len(population) // 2):
                ...
                child1, child2 = modified_order_crossover(parent1, parent2, crossover_rate)
