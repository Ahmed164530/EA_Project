...
weeks = [f"Week {i}" for i in range(1, 8)]  # 45 أسبوع
days_in_week = 6  # عدد الأيام في كل أسبوع
...
# Helper function to generate all possible slots (6 days in each week)
def all_possible_slots():
    slots = []
    for week in weeks:
        for day in range(1, days_in_week + 1):  # 6 أيام في الأسبوع
            for time_slot in time_slots:
                for venue in venues:
                    slots.append((week, f"Day {day}", time_slot, venue))
    return slots
...
def create_individual():
    ...
    used_slots = set()
    team_played_week = defaultdict(set)

    for match in matches:
        assigned = False
        random.shuffle(available_slots)
        for slot in available_slots:
            week, day, time_slot, venue = slot
            if slot in used_slots:
                continue
            team1, team2 = match
            if (week in team_played_week[team1]) or (week in team_played_week[team2]):
                continue
            schedule.append((team1, team2, week, day, time_slot, venue))  # تعديل الجدول ليتضمن الأسبوع، اليوم، والوقت
            used_slots.add(slot)
            team_played_week[team1].add(week)
            team_played_week[team2].add(week)
            assigned = True
            break
        if not assigned:
...
def fitness_function(schedule):
    ...
    for match in schedule:
        team1, team2, week, day, time_slot, venue = match  # تعديل الجدول ليتضمن الأسبوع، اليوم، والوقت
        week_index = weeks.index(week)
        day_index = int(day.split()[1]) - 1  # استخدم اليوم كرقم في الأسبوع

        team_schedule[team1].append((week_index, day_index, time_slot))
        team_schedule[team2].append((week_index, day_index, time_slot))

        venue_schedule[(week, day, time_slot, venue)].append((team1, team2))  # تعديل الجدول ليتضمن الأسبوع، اليوم، والوقت

    for team, plays in team_schedule.items():
        plays.sort()
        played_days = [(w, d, t) for w, d, t in plays]

        if len(set(played_days)) < len(played_days):  
            penalty += 50

        for i in range(1, len(played_days)):
            if played_days[i][0] == played_days[i-1][0] and played_days[i][1] == played_days[i-1][1] and played_days[i][2] == played_days[i-1][2]:
                penalty += 20  # تكرار المباراة في نفس اليوم والوقت
...
def swap_mutation(schedule, mutation_rate=0.1):
    if random.random() < mutation_rate:
        ...
        match_to_mutate[4] = random.choice(time_slots)
        match_to_mutate[5] = random.choice(venues)
...
def run_genetic_algorithm(population_size=10, max_generations=100, convergence_threshold=1e-3, max_no_improvement=10, mutation_rate = 0.1, crossover_rate=0.7):
    ...
    for generation in range(max_generations):
        ...
        for match in best_individual:
            print(f"{match[0]} vs {match[1]} on {match[2]} {match[3]} - Slot {match[4]} at {match[5]}")
